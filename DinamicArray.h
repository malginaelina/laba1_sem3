#ifndef LABA1SEM3_DINAMICARRAY_H
#define LABA1SEM3_DINAMICARRAY_H
...
public:
    ...
    DynamicArray(vector<T> items, int size);
...
template<typename T> DynamicArray<T>::DynamicArray(vector<T> items, int size) : DynamicArray(size) {
    if (size<0) {
        throw length_error("Negative size!");
    }
    this->size =size;
...
#endif //LABA1SEM3_DINAMICARRAY_H
